...
import DeleteHostelButton from "@/components/Forms/DeleteHostelButton";
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2 max-w-2xl">
          Create new hostels on this page.<br />
          You can manually add blocks and rooms to hostels here.<br />
          To add in bulk, go to the Block Config page.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {hostels.map((hostel) => (
          <Card key={hostel.id} className="hover:shadow-lg transition-all duration-300 border-t-4 border-t-indigo-600 h-full">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <CardTitle className="text-xl font-bold text-gray-800">
                  <Link href={`/dashboard/buildings/${hostel.id}`}>{hostel.name}</Link>
                </CardTitle>
